%% ----- CREATE INPUTS NEEDED TO COMPUTE ACPW METHOD ON EMIT DATA -----
...
function inputs = create_emit_inputs_ACPW_perPixel(emit, tblut_retrieval, print_libRadtran_err, pixel_num)
...
% -----------------------------
inputs.RT.sza = emit.obs.solar.zenith(pixel_num);           % degree
...
inputs.RT.phi0 = mod(emit.obs.solar.azimuth(pixel_num) + 180, 360);         % degree
...
inputs.RT.vza = double(emit.obs.sensor.zenith(pixel_num)); % values are in degrees;                        % degree
...
inputs.RT.vaz = emit.obs.sensor.azimuth(pixel_num);
% --------------------------------------------------------------
