...
if nargin~=15
    error([newline,'Should be 15 inputs: droplet effective radius, optical depth, altitude,',...
        ...
        ' the location where wc files will be saved, and the location where the mie files are saved.'], newline])
end
...
if size(re,1)>1 && size(re,2)>1
    ...
    if strcmp(parameterization_str, 'mie')==true && any(any(re<1)) || any(any(re>35))
        warning([newline,'At least one value in r_{e} is greater than 35 microns, or less than 1 micron',...
            ', which are the bounds of the',...
            ' Mie Interpolate parameterization that computes optical properties from the water cloud ',...
            'parameters. The netcdf file downloaded from LibRadTrans website includes values of re up to',...
            ' 35 microns. The acceptable range for Mie Interpolation is [1, 35] microns.',newline]);
    end
