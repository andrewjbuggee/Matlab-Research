...
function [F_norm, final_state, photon_tracking, inputs] = twoStream_monteCarlo_with_livePlot(inputs)
...
hold on
yline(tau_upper_limit,'LineWidth',1.5,'Color','black')
hold on;
grid on; grid minor
...
% for full screen
set(gcf, 'Position', [0,0,1512,900])
set(ax, 'XLimMode', 'manual', 'YLimMode', 'manual')
ylim([-1,tau_upper_limit+1])
xlim([-1, 1])
...
% Set the text box location for number of scattering events
dim1 = [0.15 0.865 0.236083824157715 0.0985912584612054];

% Set the text box location for keeping track of final states
dim2 = [0.7 0.885 0.236083824157715 0.0985912584612054];

% Set the text box defining the wavelength, ssa and g
dim3 = [0.15 0.5 0.15 0.0985912584612054];

texBox_str3 = {['$\lambda = $',num2str(inputs.mie.wavelength(1)), ' $nm$'],...
    ['$\varpi = $',num2str(inputs.ssa(1))],...
    ['$g = $',num2str(inputs.g(1))]};

t3 = annotation('textbox',dim3,'string',texBox_str3,'Interpreter','latex');
t3.Color = 'black';
t3.FontSize = 30;
t3.FontWeight = 'bold';
...
% get folder path if saving live plot as video and start file
if inputs.saveLivePlot_asVideo == true


    if strcmp(whatComputer, 'andrewbuggee')==true

        saveVideo_path = ['/Users/andrewbuggee/Documents/MATLAB/Matlab-Research/',...
            'Radiative_Transfer_Physics/Monte_Carlo/Monte_Carlo_videos/'];

    elseif strcmp(whatComputer, 'anbu8374')==true

        error([newline,'Where is the path to save videos?', newline])

    end

    % v = VideoWriter([saveVideo_path, "TwoStream_monteCarlo_simulation_18-Feb-2025.avi"]);
    % open(v);
...
for nn = 1:N_photons
    ...
    % ---------------------------
    % Create plot object
    p = zeros(1, length(depth_travelled{nn}));

    for ff = 1:length(depth_travelled{nn})-1

        % Plot photon path as line segments
        % in two-stream, the photon only moves long the y axis
        p(ff) = plot([0,0],[depth_travelled{nn}(ff), depth_travelled{nn}(ff+1)], ...
            'LineWidth',6);

        % using quiver to draw arrows
        % first two components are the x and y position of the start of the
        % arrow. The next to components are the ADDITIONS to the initial x and y
        % the last 0 turns off automatic scaling
        % p(ff) = quiver(0, depth_travelled{nn}(ff), 0,...
        %     (depth_travelled{nn}(ff+1) - depth_travelled{nn}(ff)),0, ...
        %     'LineWidth',4.5);

        hold on

        % define the plot size so it never changes
        ...
        % plot the number of scattering events
        if exist('t1', 'var')
            delete(t1)
        end

        texBox_str1 = {['Num Scattering Events = ',num2str(ff-1)]};

        t1 = annotation('textbox',dim1,'string',texBox_str1,'Interpreter','latex');
        t1.Color = 'black';
        t1.FontSize = 20;
        t1.FontWeight = 'bold';
        t1.EdgeColor = 'black';
        ...
        % save the frame if you'd like to save the entire figure as a video
        if inputs.saveLivePlot_asVideo == true
            F(ff*nn) = getframe(gcf);
        end
    ...
    texBox_str2 = {['Num Absorbed = ',num2str(absorbed)],...
        ['Num Scattered Out Top = ',num2str(scatter_out_top )],...
        ['Num Scattered Out Bottom = ',num2str(scatter_out_bottom)]};

    t2 = annotation('textbox',dim2,'string',texBox_str2,'Interpreter','latex');
    t2.Color = 'black';
    t2.FontSize = 20;
    t2.FontWeight = 'bold';
    t2.EdgeColor = 'black';
...
% save the video object, if desired
if inputs.saveLivePlot_asVideo == true
    writerObj = VideoWriter([saveVideo_path,'TwoStream_monteCarlo_simulation_', date,'.avi']);
    writerObj.FrameRate = 10;   % seconds per image
