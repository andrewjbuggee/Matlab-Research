...
parfor nn=1:N_photons
    ...
    % define a temporary variable that will count the number of photons moving
    % down each bin for each parallel worker
    local_counts_down = zeros(size(N_counts_moving_down)); % Local to each worker
    local_counts_up = zeros(size(N_counts_moving_up)); % Local to each worker
...
%             N_counts_moving_down(bins_photon_moves_through) = N_counts_moving_down(bins_photon_moves_through) +1;
            ...
            bins_photon_moves_through = find(binEdges>=photon_tau_absolute_position{nn}(tt,3) & ...
                binEdges<photon_tau_absolute_position{nn}(tt+1,3));
