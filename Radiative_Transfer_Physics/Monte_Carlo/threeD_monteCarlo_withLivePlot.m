...
function [F_norm, final_state, photon_tracking, inputs] = threeD_monteCarlo_withLivePlot(inputs)
...
figure;
plot3([-tau_z_upper_limit, tau_z_upper_limit],[0,0], [0,0], 'LineWidth',0.5,'Color','black')
hold on;
plot3([0,0], [-tau_z_upper_limit, tau_z_upper_limit], [0,0],'LineWidth',0.5,'Color','black')
plot3([0,0], [0, 0], [tau_z_lower_limit, tau_z_upper_limit],...
...
zlabel('$\tau_z$','Interpreter','latex', 'FontSize',32);
...
for nn = 1:N_photons
    ...
    % ---------------------------
    % Create plot object
    p = zeros(1, size(photon_tau_absolute_position{nn},1));

    for ff = 1:size(photon_tau_absolute_position{nn},1)-1

        % Plot photon path
        p(ff) = plot3([photon_tau_absolute_position{nn}(ff,1), photon_tau_absolute_position{nn}(ff+1,1)],...
            [photon_tau_absolute_position{nn}(ff,2), photon_tau_absolute_position{nn}(ff+1,2)], ...
            [photon_tau_absolute_position{nn}(ff,3), photon_tau_absolute_position{nn}(ff+1,3)],...
            'LineWidth',6);
        hold on
