...
set(gca, 'XTick', 1:n_vars, 'XTickLabel', var_labels, 'TickLabelInterpreter', 'latex', 'Fontsize', fntsz);
ylabel('Fractional 1\sigma uncertainty (%)', 'Interpreter', 'latex', 'Fontsize', fntsz);
...
set(gca, 'XTick', 1:n_vars, 'XTickLabel', var_labels, 'TickLabelInterpreter', 'latex', 'Fontsize', fntsz);
ylabel('Share of forward-model variance (%)', 'Interpreter', 'latex', 'Fontsize', fntsz);
ylim([0 100]);
title('(a) Which source dominates the forward-model budget','Interpreter', 'latex', 'Fontsize', fntsz);
legend(bs, 'Location', 'southoutside', 'Interpreter', 'latex', 'NumColumns', 1);
grid on
...
set(gca, 'XTick', 1:n_vars, 'XTickLabel', var_labels, 'TickLabelInterpreter', 'latex', 'Fontsize', fntsz);
ylabel('Forward-model share of total variance (%)', 'Interpreter', 'latex', 'Fontsize', fntsz);
title('(b) How much of the total retrieval error is forward model', 'Interpreter', 'latex', 'Fontsize', fntsz);
grid on
...
for i = 1:n_vars
    ...
    xlabel('retrieved \tau_c', 'Interpreter', 'latex', 'Fontsize', fntsz);
    ylabel('fractional 1\sigma (%)', 'Interpreter', 'latex', 'Fontsize', fntsz);
    title(var_labels{i}, 'Interpreter', 'latex', 'Fontsize', fntsz);
    grid on
    if i == 1, legend(lg, 'Location', 'best', 'Interpreter', 'latex', 'FontSize', 7); end
    ...
    if isempty(var_units{i})
        ylabel('absolute 1\sigma', 'Interpreter', 'latex', 'Fontsize', fntsz);
    else
        ylabel(sprintf('absolute 1\\sigma (%s)', var_units{i}), 'Interpreter', 'latex', 'Fontsize', fntsz);
    end
    grid on
end
