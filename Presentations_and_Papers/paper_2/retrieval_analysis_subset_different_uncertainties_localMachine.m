...
%% Produce same plot using remotely retireved profiles
...
for pw = 1:length(tcpw)
    ...
    for tc = 1:length(tau_c)
        ...
        for nn = 1:length(fileNames)
            ...
            % plot the retrieved droplet profile
            if strcmp('dropletRetrieval_noACPW_HySICS', fileNames(nn).name(1:30))==true

                % plot the retrieval without the above cloud column water
                % vapor retrieval
            ...
            elseif strcmp('dropletRetrieval_HySICS', fileNames(nn).name(1:23))==true
        ...
        for nn = 1:length(fileNames)
            ...
            if nn==1
                ...
                if tc==1
                    ...
                    % what was the assumed above cloud column water vapor path?
                    title(['Simulated measurements with $', num2str(plt_uncert),...
                        '\%$ uncertainty - $acpw$ = ',num2str(round(ds.GN_inputs.measurement.actpw, 2)), ' $mm$'],...
                        'Fontsize', 25, 'Interpreter', 'latex');
