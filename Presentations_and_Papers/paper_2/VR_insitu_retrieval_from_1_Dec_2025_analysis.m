%% Analyze latest retrievals using simulated HySICS measurements with VOCALS-REx
% in-situ cloud microphsyics

% By Andrew John Buggee

%% Check RSS residuals to see if they're below the convergence limit
...
% store all rss values
rss_resiudal = zeros(length(filenames_retrieval), 1);
...
for nn = 1:length(filenames_retrieval)
    ...
    if isfield(ds, 'GN_inputs')==true
        ...
        rss_resiudal(nn) = ds.GN_outputs.rss_residual(end);

        plot(ds.GN_inputs.convergence_limit, ds.GN_outputs.rss_residual(end), '.',...
            'MarkerSize', circ_size, 'MarkerFaceColor', mySavedColors(63, 'fixed'));
...
% remove nans
rss_resiudal(isnan(rss_resiudal)) = [];

% plot a one to one line
ax_lim = [0.9 * min(rss_resiudal), 1.1 * max(rss_resiudal)];
...
grid on; grid minor
xlabel('Convergence Limit', 'Interpreter','latex', 'FontSize',fnt_sz)
ylabel('RSS Residual', 'Interpreter','latex', 'FontSize', fnt_sz)
...
text(0.05, 0.95, eq_str_lwp, 'Units', 'normalized', 'FontSize', 14,...
...
text(0.05, 0.95, eq_str_tau, 'Units', 'normalized', 'FontSize', 14,...
    'VerticalAlignment', 'top', 'BackgroundColor', 'white', 'EdgeColor', 'k')
% plot legend
legend('', 'one-to-one', 'linear fit', 'Interpreter','latex', 'Location','best', 'FontSize', 20,...
    'Color', 'white', 'TextColor', 'k')
...
text(0.05, 0.95, eq_str_acpw, 'Units', 'normalized', 'FontSize', 14,...
    'VerticalAlignment', 'top', 'BackgroundColor', 'white', 'EdgeColor', 'k')
% plot legend
legend('', 'one-to-one', 'linear fit', 'Interpreter','latex', 'Location','best', 'FontSize', 20,...
    'Color', 'white', 'TextColor', 'k')
