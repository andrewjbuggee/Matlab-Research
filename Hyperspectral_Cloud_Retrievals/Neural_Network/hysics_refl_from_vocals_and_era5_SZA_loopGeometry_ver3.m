...
%   (2) Pre-computed source_flux_perIter and spec_response_perIter as sliced
%       cell arrays before the parfor, eliminating per-iteration logical
%       indexing and broadcast matrix slicing inside the loop
%   (3) Extracted only needed scalar/string fields from inputs before the
%       parfor to reduce the broadcast variable size
%   (4) Merged the two separate parfor loops (write INP + run uvspec) into a
%       single parfor that pipelines write -> run -> read -> compute per
%       iteration, eliminating the global barrier between the two phases
%
...
[inputs.RT.radiosonde_file, d] = write_ERA5_radiosonde_DAT_with_multiPixels(era5,...
...
% -----------------------------------------------------------------------
% --- Issue 2: Pre-compute per-iteration source flux and spectral      ---
% --- response slices as sliced cell arrays before the parfor.         ---
% --- This eliminates per-iteration logical indexing on source_wavelength
% --- and broadcast matrix slicing on spec_response_value inside the   ---
% --- parfor body, replacing them with cheap cell reads.               ---
% -----------------------------------------------------------------------

% define only the spec_response values so the matrix is available in the
% workspace for pre-slicing
spec_response_value = spec_response.value;

% Pre-compute the solar flux subset for each iteration
...
% Pre-compute the spectral response row for each iteration
...
% -----------------------------------------------------------------------
% --- Issue 3: Extract only the inputs fields needed inside the parfor --
% --- to reduce the broadcast variable size sent to each worker.       ---
% -----------------------------------------------------------------------
compute_reflectivity_uvSpec = inputs.RT.compute_reflectivity_uvSpec;
rte_solver_str              = inputs.RT.rte_solver;

% Build a minimal inputs struct for readUVSPEC_ver2 (only fields it needs)
inputs_parfor.RT.rte_solver = rte_solver_str;


% Pre-allocate filename cell arrays and output storage
...
% -----------------------------------------------------------------------
% --- Issue 4: Single merged parfor that pipelines                     ---
% ---   write INP -> run uvspec -> read output -> compute reflectance  ---
% --- per iteration, eliminating the global synchronisation barrier    ---
% --- between the two previously separate parfor loops.               ---
% ---                                                                  ---
