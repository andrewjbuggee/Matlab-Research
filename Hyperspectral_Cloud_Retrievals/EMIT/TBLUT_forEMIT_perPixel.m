...
function tblut_retrieval = TBLUT_forEMIT_perPixel(emit, spec_response, folder_paths, print_libRadtran_err, print_status_updates,...
...
inputs_tblut = create_emit_inputs_TBLUT_perPixel(folder_paths, emit, spec_response, print_libRadtran_err, pixel_num);
...
tblut_retrieval = leastSquaresGridSearch_EMIT(emit.reflectance.value(inputs_tblut.bands2run', pixel_num),...
    Refl_model_tblut, inputs_tblut);
