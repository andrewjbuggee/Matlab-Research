...
function [] = plot2ReflectanceFuncBands_EMIT(emit, R, inputs, pixels2use, kingORsurf)
...
if strcmp(kingORsurf, 'king')==true
    ...
    for pp = 1:num_pixels_2run
        ...
        for rr = 1:length(re)
            ...
            plot(reshape(R(pp,rr,:,1),1,[]), reshape(R(pp,rr,:,2),1,[]),...
    ...
    annotation('textbox',[0.305 0.159443176631675 0.387963790893555 0.0485714285714286],...
        'String',{['vza = ',num2str(round(emit.obs.sensor.zenith(pp), 1)), '$^{\circ}$  sza = ',...
        num2str(round(emit.obs.solar.zenith(pp), 1)), '$$^{\circ}$', ...
        '  pixel-idx = ', num2str(pixels2use.idx(pp))]},...
        'FontSize', 20,...
        'Interpreter','latex',...
        'FitBoxToText','on',...
        'Color','k',...
...
elseif strcmp(kingORsurf, 'surf')==true
    ...
    for pp = 1:num_pixels_2run
        ...
        annotation('textbox',[0.4 0.8 0.131464712269273 0.0623003194888175],...
            'String',{['vza = ',num2str(emit.sensor.zenith(index_2plot(pp))), '$^{\circ}$  sza = ',...
            num2str(emit.solar.zenith(index_2plot(pp))), '$$^{\circ}$', ...
