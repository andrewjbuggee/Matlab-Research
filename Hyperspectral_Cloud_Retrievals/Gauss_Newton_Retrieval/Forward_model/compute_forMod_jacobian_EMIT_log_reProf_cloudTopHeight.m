...
% --- Compute the forward model parameter Jacobian matrix for EMIT channels-----
...
function jacobian_fm_ln = compute_forMod_jacobian_EMIT_log_reProf_cloudTopHeight(state_vector, measurement_estimate_ln, GN_inputs,...
...
measurement_uncert = GN_inputs.measurement.uncert_frac(1)*100;  % percent


% --- compute the Jacobian about the current state vector ---
...
for xx = 1:num_forward_model_params
    ...
    GN_inputs.RT.z_topBottom = [changing_variables(xx * num_wl, num_cloud_layers+1), ...
        changing_variables(xx * num_wl, num_cloud_layers+1) - GN_inputs.RT.H];
...
% for nn = 1:num_INP_files
    ...
    lvl_str = extractBetween(wc_filenames{nn}, '_nn', '.DAT');

    % define the input file name
    inputFileName = [num2str(round(mean(changing_variables(nn, end-2:end-1)))), '_','nm',...
        '_fm_jacobian_param_', lvl_str{1}, '.INP'];
