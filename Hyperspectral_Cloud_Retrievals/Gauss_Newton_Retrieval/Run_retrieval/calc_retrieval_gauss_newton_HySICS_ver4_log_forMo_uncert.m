...
if print_status_updates==true
    ...
    for ii = 1:num_iterations
        ...
        if max_a==0 && current_guess(1)==current_guess(2)
            ...
            % Check to see if both guesses are 1 micron, the lower limit of our
            % lookup table
        ...
        elseif max_a==0 && ii>1
            ...
            % % ** Check the constrained guess **
            % % Are any values outside the acceptable range limit for the
            % % effective radius? What about the optical depth?
            % if any(exp(constrained_guesses(1,:)) >= 25) || any(exp(constrained_guesses(2,:)) <= 1)
            % 
            %     warning([newline, 'There effective radius values in the set of constrained state vector estimates',...
            %         ' that are either larger than 25 microns, or smaller than 1 micron.', newline,...
            %         'Changing the ma'])
            % end
    ...
    for ii = 1:num_iterations
        ...
        if max_a==0 && current_guess(1)==current_guess(2)
            ...
            % Check to see if both guesses are 1, the lower limit of our
            % lookup table
            if current_guess(1)==log(1) && current_guess(2)==log(1)
...
% -----------------------------------------------------------------------------
if isfield(GN_inputs.measurement, 'r_top')==true
...
elseif isfield(GN_inputs.measurement, 're_prof')==true
    ...
    percentDiff_abs =  {100 * abs((GN_inputs.measurement.re_prof - re_prof_retrieved')./GN_inputs.measurement.re_prof),...
        ...
        100 * abs((GN_inputs.measurement.actpw - retrieval(4,end))/GN_inputs.measurement.actpw)};
...
elseif isfield(GN_inputs.measurement, 're_prof')==true
    ...
    absDiff_stateVec = {abs((GN_inputs.measurement.re_prof - re_prof_retrieved')),...
        abs((GN_inputs.measurement.tau_c - retrieval(3,end))),...
        abs((GN_inputs.measurement.actpw - retrieval(4,end)))};
