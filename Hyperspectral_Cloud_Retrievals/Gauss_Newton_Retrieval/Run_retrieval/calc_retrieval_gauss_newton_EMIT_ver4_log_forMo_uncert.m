...
function [GN_output, GN_inputs] = calc_retrieval_gauss_newton_EMIT_ver4_log_forMo_uncert(GN_inputs,...
                                emit, spec_response, folder_paths, print_status_updates)
...
if GN_inputs.bands2run<285
    ...
    measurements_log = log(emit.reflectance.value(GN_inputs.bands2run,:)); % each column represents one pixel
    measurements_lin = emit.reflectance.value(GN_inputs.bands2run,:); % each column represents one pixel


else

    measurements_log = log(emit.reflectance.value);
    measurements_lin = emit.reflectance.value;
