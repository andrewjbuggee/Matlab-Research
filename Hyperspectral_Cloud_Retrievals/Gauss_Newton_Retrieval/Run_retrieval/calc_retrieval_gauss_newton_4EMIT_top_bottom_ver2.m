...
function [GN_output, GN_inputs] = calc_retrieval_gauss_newton_4EMIT_top_bottom_ver2(GN_inputs, emit, spec_response, folder_paths)
...
for ii = 1:num_iterations
    ...
    if ii==1
        ...
        measurement_estimate = compute_forward_model_4EMIT_top_bottom_ver2(current_guess, GN_inputs, spec_response.value, folder_paths);
    ...
    % For the retrieval of r_top, r_bot, tau_c, cwv
    Jacobian = compute_jacobian_4EMIT_top_bottom_ver2(current_guess, measurement_estimate, GN_inputs, spec_response.value,...
    ...
    elseif max_a==0 && ii>1
        ...
        for mm = 1:length(a)
            ...
            if constrained_guesses(1,mm)>1 && constrained_guesses(1,mm)<25 && ...
                ...
                constrained_measurement_estimate(:,mm) = compute_forward_model_4EMIT_top_bottom_ver2(constrained_guesses(:,mm),...
...
% **** compute the jacobian using the solution state ****
    ...
    Jacobian = compute_jacobian_4EMIT_top_bottom_ver2(retrieval(:,end), new_measurement_estimate, GN_inputs, spec_response.value,...
        jacobian_barPlot_flag, folder_paths);
