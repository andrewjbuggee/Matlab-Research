...
measurement_estimate = zeros(size(wavelengths2run,1), 1);

parfor ww = 1:size(wavelengths2run,1)
% for ww = 1:size(wavelengths2run,1)

    % define the input file name
    inputFileName = [num2str(mean(wavelengths2run(ww,:))), '_','nm_rTop_', num2str(round(r_top, 4)),...
        '_rBot_', num2str(round(r_bottom, 4)),'_tauC_', num2str(round(tau_c, 4)),...
        '_CWV_', num2str(round(wv_col_aboveCloud, 4)), '.INP'];
