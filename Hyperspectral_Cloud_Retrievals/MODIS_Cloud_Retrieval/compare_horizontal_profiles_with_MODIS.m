...
if strcmp(whatComputer,'anbu8374')==true
    ...
    error([newline, 'Define new MODIS data path!', newline])
...
elseif strcmp(whatComputer,'andrewbuggee')==true
    ...
    modisPath = ['/Users/andrewbuggee/Documents/MATLAB/Matlab-Research/',...
...
modisFolder = '2008_10_18/';

% ----- November 2nd at decimal time 0.607 (14:35) -----
%modisFolder = '2008_11_02/';
...
modisInputs = create_modis_inputs([modisPath,modisFolder], L1B_fileName);
...
addpath(modisInputs.saveCalculations_fileName)

if exist(modisInputs.saveCalculations_fileName, 'dir')==false
    mkdir(modisInputs.saveCalculations_fileName)    
...
if modisInputs.flags.useAdvection == true
    ...
    save([modisInputs.saveCalculations_fileName,'horz_prof_closest2MODIS_withAdvection_',...
...
if normalize_distance==true
    ...
    if vocalsRex.flag_2DC_data_is_conforming==true
        ...
        plot(norm_dist, vocalsRex.re, 'Color',C(nn,:), 'Linewidth', lineWidth);
        ...
        % if the 2DC data is non-conforming, use only the CDP data and
        % make a note of it
        plot(norm_dist, vocalsRex.re_CDP, 'Color',C(nn,:), 'Linewidth', lineWidth);
...
legend_str{1} = ['$STD_{in-situ}$ = ', num2str(round(std_val, 2)), ' $\mu m$'];
% skip one because of the mean value
legend_str{2} = ['$\left<r_e \right>_{in-situ}$ = ', num2str(round(mean_val, 2)), ' $\mu m$'];
...
for nn = 1:length(modis_re_sort)
    plot(modis_dist_sort(nn), modis_re_sort(nn), '.', 'Color', C(2,:), 'MarkerSize', 32)
end
...
legend_str{3} = ['$STD_{modis}$ = ', num2str(round(std(modis_re), 2)), ' $\mu m$'];
legend_str{4} = ['$\left<r_e \right>_{modis}$ = ', num2str(round(mean(modis_re), 2)), ' $\mu m$'];
...
% using both instruments
if vocalsRex.flag_2DC_data_is_conforming==true
    ylabel('Effective Radius ($\mu m$)', 'Interpreter','latex')
else
    % if the 2DC data is non-conforming, use only the CDP data and
    % make a note of it
    ylabel('Effective Radius ($\mu m$) - (CDP only)', 'Interpreter','latex')
end
