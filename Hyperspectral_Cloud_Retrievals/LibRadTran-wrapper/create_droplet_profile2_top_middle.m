...
% INPUTS:
%   (1) re_topMiddle - effective droplet radius (microns) - these are the
...
%   (2) z - the vertical independent variable, which is 
...
%   (3) tau - the optical depth vector

%   (4) independent varaible - a string that tells the code what the
...
%   (5) constraint - the physical constraint (string) - there are four
...
function re = create_droplet_profile2_top_middle(re_topMiddle, z, tau, independentVariable, constraint)
...
a0 = @(x) re_topMiddle(1)^(2 + 3/x);
a1 = @(x) 2*(re_topMiddle(1)^(2 + 3/x) - re_topMiddle(2)^(2 + 3/x));

% boundary conditions for r as a function of z

b0 = @(x) re_topMiddle(2)^(3/x);
b1 = @(x) 2*(re_topMiddle(1)^(3/x) - re_topMiddle(2)^(3/x));
...
elseif strcmp(constraint,'adiabatic')
    ...
    % Solve based on the input independent variable, but output both
    % curves
    if strcmp(independentVariable,'optical_depth')==true

        re.tau = (a0(x) - a1(x)*(tau./tau(end))).^(x/(2*x + 3));

        % Use the effective radius profile solved above to solve for the
        % profile as a function of z. Use the radii at cloud top and bottom
        %
        % boundary conditions for r as a function of z
        b0 = @(x) re.tau(end)^(3/x);
        b1 = @(x) re.tau(1)^(3/x) - re.tau(end)^(3/x);

        re.z = (b0(x) + b1(x) * (z - z(1))./(z(end) - z(1))).^(x/3);       % droplet profile in geometric coordniate system for an adiabatic cloud

        
    elseif strcmp(independentVariable,'altitude')==true
        
        re.z = (b0(x) + b1(x) * (zT - zT(1))./(zT(end) - zT(1))).^(x/3);                      % droplet profile in geometric coordniate system for an adiabatic cloud
    end
