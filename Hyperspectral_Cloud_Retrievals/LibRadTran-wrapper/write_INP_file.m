...
if inputs.RT.yesCloud==true
    ...
    % --------------------------------------------------------------------
    % *** Use the default mie properties table? Or the Custom table? ***
    % --------------------------------------------------------------------
    ...
    if inputs.RT.use_custom_mie_calcs == false
        ...
        if isfield(inputs.RT, 'mean_distribution_var_closest_filename') == true
            ...
            fprintf(fileID, formatSpec,'wc_properties', inputs.RT.mean_distribution_var_closest_filename, ' interpolate', ' ', '# optical properties parameterization technique');

        else

            error([newline, 'What is the name of the custom mie table?', newline])
