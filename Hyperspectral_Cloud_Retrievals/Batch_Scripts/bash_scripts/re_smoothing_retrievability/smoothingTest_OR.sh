...
# Retrievability test of vertical droplet-size structure -- ORACLES
...
#SBATCH --output=reSmoothTest_OR_%A_%a.out
#SBATCH --error=reSmoothTest_OR_%A_%a.err
#SBATCH --mail-user=anbu8374@colorado.edu
#SBATCH --mail-type=ALL
#SBATCH --array=2101-2337%20    # 237 ORACLES profiles (task id = 2100 + profile index)
...
# Offset so that array tasks 2101-2337 map to profile indices 1-237
offset=2100
...
echo "Starting MATLAB job for ORACLES profile (task ${SLURM_ARRAY_TASK_ID}) at $(date)"
...
echo "Finished MATLAB job for ORACLES profile (task ${SLURM_ARRAY_TASK_ID}) at $(date)"
