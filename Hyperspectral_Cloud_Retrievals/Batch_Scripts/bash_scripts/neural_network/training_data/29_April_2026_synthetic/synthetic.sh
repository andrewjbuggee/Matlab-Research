...
#SBATCH --array=4-900%48     # was %50 — eased to reduce scratch-filesystem contention
...
# Pre-create the MATLAB-managed parent paths so define_folderPaths_for_HySICS
# never has to create intermediate directories itself. MATLAB's mkdir on Lustre
# can fail transiently when many concurrent jobs hit the same parent path.
