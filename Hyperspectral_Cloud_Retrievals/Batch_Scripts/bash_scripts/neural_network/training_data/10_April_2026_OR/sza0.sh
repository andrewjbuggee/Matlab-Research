...
#SBATCH --job-name=create_meas_pt3_percent_OR_insitu_ERA5_trainingData_sza0
#SBATCH --output=create_meas_pt3_percent_OR_insitu_ERA5_trainingData_sza0_%A_%a.out
#SBATCH --error=create_meas_pt3_percent_OR_insitu_ERA5_trainingData_sza0_%A_%a.err
#SBATCH --mail-user=anbu8374@colorado.edu
#SBATCH --mail-type=ALL
#SBATCH --array=101-339%16    # throttling to 6 concurrent jobs to avoid overwhelming the filesystem with too many simultaneous MATLAB startups and libRadtran runs
...
# define the offset for the job array so that the measurement index ranges from 1 to 239 instead of 101 to 340 (since we are only processing 239 measurements with valid SZA and in situ data)
offset=100
