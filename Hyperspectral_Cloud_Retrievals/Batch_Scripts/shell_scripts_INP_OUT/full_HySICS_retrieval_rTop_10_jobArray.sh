...
# Each job processes 4 files sequentially, using 16 CPUs for each file's parfor
...
#SBATCH --qos=normal
#SBATCH --job-name=retrieval_hybrid_%A_%a
#SBATCH --output=retrieval_hybrid_%A_%a.out
#SBATCH --error=retrieval_hybrid_%A_%a.err
#SBATCH --mail-user=anbu8374@colorado.edu
#SBATCH --mail-type=ALL
#SBATCH --array=1-25        # 25 jobs × 4 files each = 100 files
...
for (( i=START_IDX; i<=END_IDX; i++ )); do
    if [ $i -lt ${#ALL_FILES[@]} ]; then
        ...
        echo "Processing file $((i+1)): ${CURRENT_FILE}"
        ...
        time matlab -nodesktop -nodisplay -r "addpath(genpath('/projects/anbu8374/Matlab-Research')); addpath(genpath('/scratch/alpine/anbu8374/HySICS/INP_OUT/')); addpath(genpath('/scratch/alpine/anbu8374/Mie_Calculations/')); clear variables; addLibRadTran_paths; current_file='${CURRENT_FILE}'; current_file_path='${CURRENT_FILE_PATH}'; num_workers=${SLURM_CPUS_PER_TASK}; [folder_paths, which_computer] = define_folderPaths_for_HySICS('${CURRENT_FILE}'); print_status_updates = true; print_libRadtran_err = true; test_retrieval_HySICS_no_msgs_orDefinedFolder; exit"
        
        echo "Completed file: ${CURRENT_FILE} at $(date)"
    fi
done
