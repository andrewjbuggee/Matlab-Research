...
% This script retrieves 3 variables: ln(r_top), ln(r_bot), and ln(tau_c)
...
function [tblut_retrieval, GN_inputs, GN_outputs] = run_retrieval_dropletProfile_HySICS_ver4_logNewCov_noACPW_15(filenames, folder_paths, print_status_updates, print_libRadtran_err)
...
for ff = 1:length(filenames)
    ...
    GN_inputs.RT.waterVapor_column = 15;   % mm - milimeters of water condensed in a column
    ...
    % Create inputs to retrieve log(r_top), log(r_bot), log(tau_c)
    GN_inputs = create_model_prior_covariance_HySICS_ver4_log_newCov_noACPW(GN_inputs, tblut_retrieval, use_TBLUT_estimates);
    ...
    % --------------------------------------------------------------
    [GN_outputs, GN_inputs] = calc_retrieval_gauss_newton_HySICS_log_newCov(GN_inputs, simulated_measurements, folder_paths, print_status_updates);
    % --------------------------------------------------------------
    % --------------------------------------------------------------
