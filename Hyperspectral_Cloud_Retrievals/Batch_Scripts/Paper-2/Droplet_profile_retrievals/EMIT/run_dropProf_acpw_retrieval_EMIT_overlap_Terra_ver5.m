...
[overlap_pixels, emit, modis, folder_paths] = findOverlap_pixels_EMIT_Terra_coincident_data(folder_paths,...
    criteria, plot_data);
...
if plot_figures == true
    ...
    % ** Plot with RGB Image **
    % fig = plot_instrument_footprints(modis, emit, [], overlap_pixels, options);
    % fig1 = plot_instrument_footprints_2(modis, emit, [], overlap_pixels, options);
    % [fig1, ax1] = plot_instrument_footprints_3(modis, emit, [], [], overlap_pixels, options);
    [fig1, ax1] = plot_instrument_footprints_4(modis, emit, [], [], overlap_pixels, options);
    ...
    % fig2 = plot_instrument_footprints_2(modis, emit, [], overlap_pixels, options);
    fig2 = plot_instrument_footprints_3(modis, emit, [], [], overlap_pixels, options);
    ...
    if save_figures==true
        ...
        ax1.Title.String = '';
        f = gcf;
        saveas(f,[folderpath_figs,'EMIT Scene without MODIS context - ',...
        ...
        ax1.Title.String = '';
        exportgraphics(f,[folderpath_figs,...
            'EMIT Scene without MODIS context - ',...
...
for pp = 1:length(overlap_pixels.modis.linear_idx)
    ...
    [GN_inputs, GN_outputs, tblut_retrieval, acpw_retrieval, folder_paths] = retrieve_dropProf_acpw_EMIT_Terra_singlePix_ver2(emit,...
        modis, era5, overlap_pixels, folder_paths, print_libRadtran_err, print_status_updates, pp);
