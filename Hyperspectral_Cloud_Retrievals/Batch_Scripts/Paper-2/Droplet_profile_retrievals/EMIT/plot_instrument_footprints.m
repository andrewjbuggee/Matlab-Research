...
%   pixels.amsr.col = 50;
%   
%   fig_handle = plot_instrument_footprints(modis, emit, AMSR, pixels);
%
...
%   
%   fig_handle = plot_instrument_footprints(modis_filtered, emit_filtered, amsr_filtered, pixels);
%
% Example usage with RGB backdrop:
%   % First create the RGB image
%   [rgb_img, rgb_lat, rgb_lon] = create_modis_true_color(modis_full);
%   
%   % Then plot with RGB backdrop
%   options.show_rgb = true;
%   options.rgb_image = rgb_img;
%   options.rgb_lat = rgb_lat;
%   options.rgb_lon = rgb_lon;
%   
%   fig_handle = plot_instrument_footprints(modis, emit, AMSR, pixels, options);
%
...
if ~isfield(options, 'colors')
    % Default colors: MODIS (blue), EMIT (red), AMSR (green)
...
%% Plot RGB image backdrop if requested
if options.show_rgb && isfield(options, 'rgb_image') && ...
    ...
    try
        ...
        latlim = [min(options.rgb_lat(:), [], 'omitnan'), ...
                 max(options.rgb_lat(:), [], 'omitnan')];
        lonlim = [min(options.rgb_lon(:), [], 'omitnan'), ...
        ...
        % For georasterref, LatitudeLimits should be [SOUTH, NORTH]
        % This is INDEPENDENT of the image orientation
        lat_min = min(rgb_lat(:), [], 'omitnan');
        lat_max = max(rgb_lat(:), [], 'omitnan');
        lon_min = min(rgb_lon(:), [], 'omitnan');
        lon_max = max(rgb_lon(:), [], 'omitnan');
        
        latlim = [lat_min, lat_max];  % [south, north]
        lonlim = [lon_min, lon_max];  % [west, east]
...
if ~isempty(emit)
    ...
    for ii = 1:num_emit
        ...
        if options.show_centers
            ...
            if using_map_axes
                plotm(center_lat, center_lon, 's', ...
                    'MarkerFaceColor', options.colors.emit, ...
                    'MarkerEdgeColor', 'k', 'MarkerSize', 8);
            else
                geoplot(center_lat, center_lon, 's', ...
                    'MarkerFaceColor', options.colors.emit, ...
                    'MarkerEdgeColor', 'k', 'MarkerSize', 8);
            end
        end
...
if ~isempty(amsr)
    ...
    for ii = 1:num_amsr
        ...
        if options.show_centers
            ...
            if using_map_axes
                plotm(center_lat, center_lon, 'd', ...
                    'MarkerFaceColor', options.colors.amsr, ...
                    'MarkerEdgeColor', 'k', 'MarkerSize', 10);
            else
                geoplot(center_lat, center_lon, 'd', ...
                    'MarkerFaceColor', options.colors.amsr, ...
                    'MarkerEdgeColor', 'k', 'MarkerSize', 10);
            end
        end
