...
function [vert_profs_vapor] = find_verticalProfiles_waterVapor_VOCALS_REx(vocalsRex, LWC_threshold, Nc_threshold)
...
for nn = 1:length(vert_profs_vapor)
    ...
    vert_profs_vapor(nn).LWC_threshold = LWC_threshold;            % g/m^3

    % Store the Nc threshold used
    vert_profs_vapor(nn).Nc_threshold = Nc_threshold;            % g/m^3
